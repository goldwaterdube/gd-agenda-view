<script>
    ...
    $: {
        ...
        for (let event of $_events) {
            if (event.allDay && event.kind !== 'anniversary' && event.display !== 'background' && eventIntersects(event, start, end, resource)) {
    ...
    $: iChunks = $_iEvents.map(event => {
        let chunk;
        if (event && event.allDay && event.kind !== 'anniversary' && eventIntersects(event, start, end, resource)) {
